...
class StudentUSocket(StudentUSocketBase):
    ...
    ***if the current state is SYN_SENT, it calls handle_synsent() with the new segment.
    ...
    ## End of Stage 1 ##
      ...
      if self.acceptable_seg(seg, payload):
        ...
        # if self.rcv.nxt |LE| seg.seq and seg.seq |LE| (self.rcv.nxt |PLUS| self.rcv.wnd):
    ...
    while (not self.rx_queue.empty()):
      packet_tuple = self.rx_queue.peek()
      if packet_tuple[0] |GT| self.rcv.nxt:
        self.set_pending_ack()
        break
      else:
        self.rx_queue.pop()
        ...
        self.handle_accepted_seg(seg, data)
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    ## Start of Stage 2 ##
    if self.state == ESTABLISHED or self.state == FIN_WAIT_1 or self.state == FIN_WAIT_2:
      if len(payload) > 0:
